...
import '../../model/fetchAssignAreaLocation.dart';
import '../resource/app_colors.dart';
import '../resource/app_padding.dart';
import 'edit_assign_area_location.dart';

class ViewAllAssignAreaLocation extends StatefulWidget {
  const ViewAllAssignAreaLocation({Key? key}) : super(key: key);

  @override
  State<ViewAllAssignAreaLocation> createState() => _ViewAllAssignAreaLocationState();
}

class _ViewAllAssignAreaLocationState extends State<ViewAllAssignAreaLocation> {
  List<FetchAssignAreaLocationModel> workerData = [];

  int currentPage = 1;
  int itemsPerPage = 5;

  List<FetchAssignAreaLocationModel> getPaginatedData() {
  ...
  Future<void> fetchDataFromDatabase() async {
    final response = await http.get(Uri.parse(API.fetchAllAssignedAreaLocation));
    if (response.statusCode == 200) {
      ...
      if (jsonResponse is Map<String, dynamic> &&
        ...
        List<FetchAssignAreaLocationModel> newData =
        (jsonResponse['data'] as List<dynamic>)
            .map((item) => FetchAssignAreaLocationModel.fromJson(item))
            .toList();
                ...
                if (parsedAreaLocationId != null) {
                  try {
                    final response = await http.delete(
                      Uri.parse(
                          'http://${Config.ipAddress}/kewasco_api/modules/deleteAssignAreaLocation.php?id=$parsedAreaLocationId'),
                    );
  ...
  Future<void> confirmEditTask(
      BuildContext context, int areaLocationId, String teamLeaderName,String areaLocationName) async {
    return showDialog(
      context: context,
      builder: (BuildContext context) {
        return EditAssignAreaLocationModal(
            areaLocationId,
            teamLeaderName,
            areaLocationName
        );
  ...
  Widget build(BuildContext context) {
    List<FetchAssignAreaLocationModel> paginatedData = getPaginatedData();
                                        ...
                                        children: [
                                          IconButton(onPressed: () => confirmDeleteTask(
                                              ...
                                              paginatedData[index].id?.toString() ??
                                                  'N/A'), icon: Icon(Icons.delete_forever,color: Colors.red,)),
