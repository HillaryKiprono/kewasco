...
import '../../model/fetchWorkersModel.dart';
import '../resource/app_colors.dart';
import 'edit_worker.dart';

class ViewAllWorkers extends StatefulWidget {
  const ViewAllWorkers({Key? key}) : super(key: key);

  @override
  State<ViewAllWorkers> createState() => _ViewAllWorkersState();
}

class _ViewAllWorkersState extends State<ViewAllWorkers> {
  
  List<FetchWorkerModel> workerData = [];

  int currentPage = 1;
  int itemsPerPage = 5;

  List<FetchWorkerModel> getPaginatedData() {
  ...
  Future<void> fetchDataFromDatabase() async {
    final response = await http.get(Uri.parse(API.fetchAllWorkers));
    if (response.statusCode == 200) {
      final dynamic jsonResponse = json.decode(response.body);
      print(jsonResponse); // Add this line to print the response

      if (jsonResponse is Map<String, dynamic> && jsonResponse.containsKey('data')) {
        List<FetchWorkerModel> newData = (jsonResponse['data'] as List<dynamic>)
            .map((item) => FetchWorkerModel.fromJson(item))
            .toList();
                ...
                if (parsedWorkerId != null) {
                  try {
                    final response = await http.delete(
                      Uri.parse('http://${Config.ipAddress}/kewasco_api/modules/delete_worker.php?id=$parsedWorkerId'),
                    );
  ...
  Widget build(BuildContext context) {
    List<FetchWorkerModel> paginatedData = getPaginatedData();
                              ...
                              rows: List.generate(
                                paginatedData.length,
                                  ...
                                  cells: [
                                    ...
                                    DataCell(
                                      Text(paginatedData[index].workerName),
                                    ),
                                    DataCell(
                                      Container(
                            ...
                            children: [
                              ...
                              IconButton(
                                ...
                                onPressed: () {
                                  setState(() {
                                    int totalPage =
                                    (workerData.length / itemsPerPage).ceil();
                                    currentPage =
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('View All Workers'),
        centerTitle: true,
      ),
